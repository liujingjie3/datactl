package com.zjlab.dataservice.config.mybatis;
		...
		if (SqlCommandType.INSERT == sqlCommandType) {
			...
			for (Field field : fields) {
				...
				try {
					...
					//注入部门编码
					if ("departNames".equals(field.getName())) {
						field.setAccessible(true);
						Object localDepartNames = field.get(parameter);
						field.setAccessible(false);
						if (localDepartNames == null || "".equals(localDepartNames)) {
							// 获取登录用户信息
							if (sysUser != null) {
								field.setAccessible(true);
								field.set(parameter, sysUser.getDepartNames());
								field.setAccessible(false);
							}
						}
					}
