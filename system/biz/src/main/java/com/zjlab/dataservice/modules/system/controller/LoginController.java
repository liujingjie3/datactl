package com.zjlab.dataservice.modules.system.controller;
...
import com.zjlab.dataservice.common.system.util.JwtUtil;
import com.zjlab.dataservice.common.system.vo.LoginUser;
import com.zjlab.dataservice.common.util.*;
import com.zjlab.dataservice.common.util.encryption.EncryptedString;
import com.zjlab.dataservice.config.JeecgBaseConfig;
import com.zjlab.dataservice.modules.base.service.BaseCommonService;
import com.zjlab.dataservice.modules.system.entity.OuathUserInfo;
import com.zjlab.dataservice.modules.system.entity.SysDepart;
import com.zjlab.dataservice.modules.system.entity.SysRoleIndex;
import com.zjlab.dataservice.modules.system.entity.SysUser;
import com.zjlab.dataservice.modules.system.model.SysLoginModel;
import com.zjlab.dataservice.modules.system.service.*;
import com.zjlab.dataservice.modules.system.service.impl.SysBaseApiImpl;
import com.zjlab.dataservice.modules.system.util.RandImageUtil;
...
public class LoginController {
    ...
    private static final String login = "TJ1001";
    private static final String sign = "TJ1002";
    private static final String forget = "TJ1003";
...
//        }
    ...
    @ApiOperation("OA统一认证接口")
    @GetMapping("/oaLogino")
    public Result<JSONObject> oaLoginOuathCode(HttpServletRequest request, @RequestParam(value = "code") String code) throws UnsupportedEncodingException {
        Result<JSONObject> result = new Result<JSONObject>();
        //请求access_token
        ...
        redirect = this.addParameter(redirect, "grant_type", LoginController.AUTHORIZATION_CODE, false);
        ...
        ResponseEntity<String> responseEntity = restTemplate.postForEntity(redirect,null, String.class,null,null);

        log.info("Token: " + responseEntity.getBody());
        String s = responseEntity.getBody();
        String act = getParam(s, "access_token");
        String refresh_token = getParam(s, "refresh_token");

        ResponseEntity<OuathUserInfo> result_body = restTemplate.exchange(this.profileEndpoint + "?" + "access_token=" + act, HttpMethod.POST, null, OuathUserInfo.class);
        log.info("userInfo:" + result_body.getBody());

        OuathUserInfo ouathUserInfo = result_body.getBody();
//        String username = ouathUserInfo.getRealname();
        String uid = ouathUserInfo.getUid();

        LambdaQueryWrapper<SysUser> queryWrapper = new LambdaQueryWrapper<>();
        queryWrapper.eq(SysUser::getId, uid);
        SysUser sysUser = sysUserService.getOne(queryWrapper);
        //update-end-author:wangshuai date:20200601 for: 登录代码验证用户是否注销bug，if条件永远为false
        result = sysUserService.checkUserIsEffective(sysUser);
        if (!result.isSuccess()) {
        ...
        userInfo(sysUser, result);
//        String mobile = ouathUserInfo.getMobile();
//        JSONObject obj = new JSONObject(new LinkedHashMap<>());
//        obj.put("userInfo", sysUser);
        //1.生成token
//        String token = JwtUtil.sign(username, mobile);
//        //设置token缓存有效时间
...
//    @GetMapping("/oaLoginc")
//    public Result<JSONObject> oaLoginc(HttpServletRequest request) {
//
//        Result<JSONObject> result = new Result<JSONObject>();
//        Assertion assertion = (Assertion) request.getSession().getAttribute(AbstractCasFilter.CONST_CAS_ASSERTION);
//        String username = assertion.getPrincipal().getName();
//        log.info(assertion.getPrincipal().toString());
//
//        LambdaQueryWrapper<SysUser> queryWrapper = new LambdaQueryWrapper<>();
//        queryWrapper.eq(SysUser::getUsername, username);
//        SysUser sysUser = sysUserService.getOne(queryWrapper);
...
//                b = DySmsHelper.sendSms(mobile, obj, DySmsEnum.REGISTER_TEMPLATE_CODE);
                b = DySmsHelper.sendSmsZj(mobile, obj, sign);
            } else {
                //登录模式，校验用户有效性
                SysUser sysUser = sysUserService.getUserByPhone(mobile);
                result = sysUserService.checkUserIsEffective(sysUser);
...
//                    b = DySmsHelper.sendSms(mobile, obj, DySmsEnum.LOGIN_TEMPLATE_CODE);
                    b = DySmsHelper.sendSmsZj(mobile, obj, login);
                } else if (CommonConstant.SMS_TPL_TYPE_2.equals(smsmode)) {
                    //忘记密码模板
//                    b = DySmsHelper.sendSms(mobile, obj, DySmsEnum.FORGET_PASSWORD_TEMPLATE_CODE);
                    b = DySmsHelper.sendSmsZj(mobile, obj, forget);
                }
            }
...
//        obj.put("sysAllDictItems", sysDictService.queryAllDictItems());
    ...
    private String getParam(String s, String param) {
        String ss = s.substring(s.indexOf("{") + 1, s.indexOf("}") - 1);
        String[] split = ss.split(",");
        Map<String, String> map = new HashMap<>();
        for (String sss : split
        ) {
            String[] split1 = sss.split(":");
            map.put(split1[0].substring(1, split1[0].length() - 1), split1[1].substring(1, split1[1].length() - 1));
        }
